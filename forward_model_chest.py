...
def get_link(sdk):
    ...
    X_RecocapLink9 = np.eye(4)
    X_RecocapLink9[:3, :3] = R.from_quat(pose24_np[9]).as_matrix()
    X_RecocapLink9[1, 3] = 1.25  # chest2ground height 

    X_RecocapLink16 = np.eye(4)
    X_Link9Link16 = np.eye(4)
    X_Link9Link16[:3, :3] = R.from_quat(pose24_np[16]).as_matrix()
    X_Link9Link16[0, 3] = 0.25  # shoulder width 
    X_Link9Link16[1, 3] = 0.25  # shoulder height 
...
def main():
    ...
    def pose_msg_callback(self: rebocap_ws_sdk.RebocapWsSdk, tran: list, pose24: list, static_index: int, ts: float):
        print(f'X_WorldLink20: \n{X_WorldLink20}\n')
        print(f'X_WorldLink21: \n{X_WorldLink21}\n')
        # message = socket.recv()
        # 将 X_RecocapLink20 通过 zmq 发送
        # socket.send(b'ID_WorldLink20' + X_WorldLink20.tobytes())
        # socket.send(b'ID_WorldLink21' + X_WorldLink21.tobytes())
        pass
